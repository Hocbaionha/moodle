...
class edit_course_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('filepicker', 'popupimg', get_string('popupimg','local_school'), null,
                   array('maxbytes' => $maxbytes, 'accepted_types' => array('web_image')));
        //edx_url
        $mform->addElement('text', 'edx_url', "EDX link", $attributes);
        $mform->setType('edx_url', PARAM_TEXT);
        $mform->addElement('editor', 'edx_desc', "EDX description");
        $mform->setType('edx_desc', PARAM_RAW);
