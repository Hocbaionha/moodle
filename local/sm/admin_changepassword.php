...
require_once('admin_change_password_form.php');
...
$PAGE->set_url('/local/sm/admin_changepassword.php', array('email' => $email));
...
// if(true){
    ...
    if (empty($SESSION->wantsurl)  or stripos(str_replace('https://', 'http://', $SESSION->wantsurl), str_replace('https://', 'http://', $CFG->wwwroot . '/local/sm/admin_changepassword.php')) === 0) {
        $returnto = "$CFG->wwwroot/admin/user.php";
    } else {
...
if (!isloggedin() or isguestuser()) {
    if (empty($SESSION->wantsurl)) {
        $SESSION->wantsurl = $CFG->wwwroot . '/local/sm/admin_changepassword.php';
    }
...
$mform = new admin_login_change_password_form();
...
if ($mform->is_cancelled()) {
    ...
    //AnhNN call firebase
    firebasechangepass($data->email, $data->newpassword1);
...
function firebasechangepass($email, $newPassword)
{
    global $USER, $CFG;
    $url = $CFG->changepass;
    if (!isset($url)) {
        $url = "https://id.hocbaionha.com/update-user";
    }
    $apiKey = "o91xiy8qod1o4ebsyv035y5wssn6jmb493bd7hrn901yreninwsgvns49f69nmai";
